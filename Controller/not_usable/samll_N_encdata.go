...
func main() {
	...
	logN := 11
	primeGen := ring.NewNTTFriendlyPrimesGenerator(19, uint64(math.Pow(2, float64(logN)+1)))
	...
	params, _ := bgv.NewParametersFromLiteral(bgv.ParametersLiteral{
		LogN:             logN,
		LogQ:             []int{21, 21}, // 128bit security 맞추기 위해 Q가 줄어듦 QP에 맞춰야대서
	...
	for {
		...
		if len(totalData) >= 65870 { // 예시로 131406 크기만큼 받으면 종료
			break
		}
	}
	...
	err = ct0.UnmarshalBinary(totalData[:65870])
	if err != nil {
		// 오류 로그 출력
